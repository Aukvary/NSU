...
node* rightRotate(node *y) {
    ...
    y->height = CHUNK_SIZE(height(y->left), height(y->right)) + 1;
    x->height = CHUNK_SIZE(height(x->left), height(x->right)) + 1;
...
node* push(node* node, int val, int *added) {
    ...
    node->height = 1 + CHUNK_SIZE(height(node->left), height(node->right));
...
node* pop(node* head, int val, int *removed) {
    ...
    head->height = 1 + CHUNK_SIZE(height(head->left), height(head->right));
