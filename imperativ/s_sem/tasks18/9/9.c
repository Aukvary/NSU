...
int main() {
    ...
    static char T_str[500005];
    static char P_str[100005];

    if (fscanf(fin, "%s %s", T_str, P_str) < 2) {
        fprintf(fout, "-1\n");
        return 0;
    }
    ...
    for (int i = 0; i < n; i++) s[i] = T_str[i] - 'a' + 1;
    s[n] = s[n + 1] = s[n + 2] = 0;
    ...
    while (low <= high) {
        ...
        if (strncmp(T_str + SA[mid], P_str, m) >= 0) {
            first = mid;
            high = mid - 1;
        } else {
            low = mid + 1;
    ...
    if (first == -1 || strncmp(T_str + SA[first], P_str, m) != 0) {
        fprintf(fout, "-1\n");
    } else {
        int last = first;
        low = first, high = n - 1;
        ...
        fprintf(fout, "%d\n", count);
        for (int i = 0; i < count; i++) {
            fprintf(fout, "%d%s", results[i], (i == count - 1 ? "" : " "));
        }
        fprintf(fout, "\n");
        free(results);
    }
